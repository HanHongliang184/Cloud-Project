...
void Book::renameFile()
{
    ...
    if (NULL == pItem) {
        ...
        // qDebug() << strOldName;
        QString strNewName = QInputDialog::getText(this, "重命名文件", "请输入新的文件：");
        // qDebug() << strNewName;
        if (!strNewName.isEmpty()){
            ...
            memcpy(pdu->caMsg, strCurPath.toStdString().c_str(), strCurPath.size());
            // qDebug() << pdu->caData << pdu->caData+32 << pdu->caMsg;

            TcpClient::getInstance().getTcpSocket().write((char*)pdu, pdu->uiPDULen);
            free(pdu);
            pdu = NULL;
        } else {
            QMessageBox::warning(this, "重命名文件", "新的文件不能为空！");
        }
...
void Book::returnPre()
{
    ...
    if (strCurPath == strRootPath) {
        ...
        TcpClient::getInstance().strCurPath(strCurPath);
...
void Book::uploadFile()
{
    ...
    if (!m_strUploadFilePath.isEmpty()) {
        // aa\bb\cc 5
        int index = m_strUploadFilePath.lastIndexOf('/');//从后向前找
